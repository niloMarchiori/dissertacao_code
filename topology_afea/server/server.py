...
def server():
    ...
    # connect on queue
    controller = Controller(min_trainers=min_trainers, num_rounds=nun_rounds, aggregator=aggregator, clients_args=clients_args)
    ...
    while controller.get_current_round() != nun_rounds:
        ...
        select_trainers = [t for t in select_trainers_bool.keys() if select_trainers_bool[t]]
        ...
        for t in trainer_list:
            if t in select_trainers:
                ...
                m = json.dumps({'id': t, 
                                ...
                                'tgt_acc': float(tgt_acc[t]),
                                'n_epochs': int(n_epochs[t])}).replace(' ', '')
